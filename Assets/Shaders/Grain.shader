Shader "Hidden/Grain"
{
    Properties
    {
		...
		_Strength("Noise Strenth", Float) = 0.05
		_Aspect("Aspect Ration", Float) = 1.778
    }
    SubShader
    {
        // No culling or depth
        ...
        Pass
        {
			...
			float rand(float2 pos) {
				return(frac(sin(dot(pos + _Time.y, float2(12.9898f,78.233f))))*43758.543123f);
			}
			...
			float perlin2D(float2 pixel) {
				...
				float2 pos11 = pos00 + float2(1.0f, 10.f);
